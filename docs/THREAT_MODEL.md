...
| **A defect in the JNI boundary on Android** | From Phase 5 the Android client reaches the core across FFI. A Rust panic unwinding across that boundary is undefined behaviour, so every exported function catches; a guardrail counts exports against `catch_unwind` wrappers and fails the build if one arrives without it. What that does **not** cover is the JNI marshalling itself, which — uniquely in this project — has never been executed. CI compiles both sides and links the native library for all four ABIs, and the decisions behind the boundary are under test on the host; but no JVM has ever called across it, because the unit tests are plain JVM tests that never load the library and the debug APK ships without one. The design response was to make that surface one function rather than thirty-five (D-048), with every decision behind it under test, but "compiles, and is small and reviewed" is not "run". Treat the Android client as the least-exercised code here until it has run on hardware. |
